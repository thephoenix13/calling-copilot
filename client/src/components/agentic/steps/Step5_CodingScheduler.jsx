...
export default function Step5_CodingScheduler({ session, authFetch, onComplete, onRefresh }) {
  ...
  useEffect(() => {
    authFetch(`${BACKEND_URL}/coding-assessments`)
      .then(r => r.json())
      .then(d => setAssessments(d.assessments || []))
      .catch(() => setAssessments([]))
      .finally(() => setLoading(false));

    setSelected(new Set(passing.map(c => c.id)));
  }, []);
  ...
  return (
    <div className="sw-step-page">
      <div className="sw-step-header">
        <h2 className="sw-step-title">Step 5 — Coding Assessment</h2>
        <p className="sw-step-desc">
          Select a coding assessment and invite shortlisted candidates. This step is optional — click Continue to skip.
        </p>
      </div>
      ...
      <div className="sw-sched-grid">
        ...
        <div className="sw-content-card">
          <div className="sw-sched-section-title">Coding Assessment</div>

          {loading ? (
            <div style={{ fontSize: 13, color: 'var(--text-3)', padding: '8px 0' }}>Loading assessments…</div>
          ) : assessments.length === 0 ? (
            <div style={{ fontSize: 13, color: 'var(--text-3)', lineHeight: 1.6 }}>
              No coding assessments found. Go to <strong>Coding Assessments</strong> under Evaluation to create one first.
            </div>
          ) : (
            <select
            ...
            >
              ...
              {assessments.map(a => (
                <option key={a.id} value={a.id}>
                  {a.title} · {a.question_count ?? 0} questions · {a.time_limit_min || 60} min
                </option>
              ))}
            </select>
          )}
          ...
          {selectedAssessment && (
            <div className="sw-sched-iv-meta" style={{ marginTop: 10 }}>
              ...
              <span className="sw-sched-meta-chip">{selectedAssessment.time_limit_min || 60} min</span>
